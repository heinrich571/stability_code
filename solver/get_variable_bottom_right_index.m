function Bottom_Right_Index = get_variable_bottom_right_index(Variable_Name, Nx, Ny)
...
% 
% Output:
%   Bottom_Right_Index  [int]       Column index of specified variable at
%                                   the bottom-right corner of the domain
%                                   in the eigenvalue problem matrices.

% Input validation
if ~isnumeric(Nx)
    error('get_variable_bottom_right_index:NxNotNumeric', ...
          'Nx must be numeric.')
end
if ~isnumeric(Ny)
    error('get_variable_bottom_right_index:NyNotNumeric', ...
          'Ny must be numeric.')
end
if mod(Nx, 1) ~= 0 % Check wether Nx is an integer
    error('get_variable_bottom_right_index:NxNotInteger', ...
          ['Nx equals ' num2str(Nx) ', but it must be an integer.'])
end
if mod(Ny, 1) ~= 0 % Check wether Ny is an integer
    error('get_variable_bottom_right_index:NyNotInteger', ...
          ['Ny equals ' num2str(Ny) ', but it must be an integer.'])
end

% Calculate the bottom-right index of the specified variable
