function Bottom_Indices = get_operator_bottom_indices(Nx, Ny)

% Description:
%   This function returns the row indices of the operator matrix which
%   correspond to the bottom boundary of the domain.
% 
% Input:
...
% 
% Output:
%   Bottom_Indices   [int]      Row indices of the operator at the bottom
%                               boundary of the domain.

% Input validation
if ~isnumeric(Nx)
    error('get_operator_bottom_indices:NxNotNumeric', ...
          'Nx must be numeric.')
end
if ~isnumeric(Ny)
    error('get_operator_bottom_indices:NyNotNumeric', ...
          'Ny must be numeric.')
end
if mod(Nx, 1) ~= 0 % Check wether Nx is an integer
    error('get_operator_bottom_indices:NxNotInteger', ...
          ['Nx equals ' num2str(Nx) ', but it must be an integer.'])
end
if mod(Ny, 1) ~= 0 % Check wether Ny is an integer
    error('get_operator_bottom_indices:NyNotInteger', ...
          ['Ny equals ' num2str(Ny) ', but it must be an integer.'])
end

% Calculate the bottom indices of an operator
