function Left_Indices = get_equation_whole_left_indices(Equation_Name, Nx, Ny)

% Description:
%   This function returns the row indices of the equation matrix of the
%   eigenvalue problem which correspond to the whole left side of the
%   domain.
% 
% Input:
...
% 
% Output:
%   Left_Indices    [int]       Row indices of the equation matrix at the
%                               whole left side of the domain.

% Input validation
if ~isnumeric(Nx)
    error('get_equation_whole_left_indices:NxNotNumeric', ...
          'Nx must be numeric.')
end
if ~isnumeric(Ny)
    error('get_equation_whole_left_indices:NyNotNumeric', ...
          'Ny must be numeric.')
end
if mod(Nx, 1) ~= 0 % Check wether Nx is an integer
    error('get_equation_whole_left_indices:NxNotInteger', ...
          ['Nx equals ' num2str(Nx) ', but it must be an integer.'])
end
if mod(Ny, 1) ~= 0 % Check wether Ny is an integer
    error('get_equation_whole_left_indices:NyNotInteger', ...
...
Left_Indices  = (Top_Right_Index - 1) + ((jm + 1 - 1) * Ny + 1 : Nx * Ny);
